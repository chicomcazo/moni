...
async function fixItemNames(rawNames: string[]): Promise<string[]> {
...
O OCR frequentemente erra letras — corrija para o produto real mais provável.
Exemplos: "OVD JUNB GRENCO" → "OVO JUMBO BRANCO", "ERWILN LONG" → "ERVILHA LONGA", "BATA DOCE" → "BATATA DOCE", "SACOLA VERO VALORION" → "SACOLA VERDE VALGRION".
Mantenha curto e direto.
Retorne JSON: {"names": ["nome1", "nome2", ...]} na mesma ordem.`,
      },
      {
        role: "user",
        content: JSON.stringify(rawNames),
      },
    ],
  ...
  if (!content) return rawNames;

  const parsed = JSON.parse(content) as { names: string[] };
  return parsed.names?.length === rawNames.length ? parsed.names : rawNames;
}
...
export async function extractReceiptData(
  ...
  // Step 1: Mistral OCR endpoint (fetch direto, sem SDK)
  ...
  if (!ocrRes.ok) {
    ...
    throw new Error(`Mistral OCR error ${ocrRes.status}: ${errBody.slice(0, 200)}`);
  }
  ...
  // Step 2: Parse números e dados (código, sem API)
  ...
  // Step 3: GPT corrige só os nomes errados do OCR
  const fixedNames = await fixItemNames(parsed.items.map((i) => i.raw_name));
